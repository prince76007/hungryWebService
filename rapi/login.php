...
$chek = $con->query("select * from rider where (mobile='".$mobile."' or email='".$mobile."') and status = 1 and password='".$password."'");
$status = $con->query("select * from rider where status = 1");
if($status->num_rows !=0)
{
if($chek->num_rows != 0)
{
    $c = $con->query("select * from rider where (mobile='".$mobile."' or email='".$mobile."')  and status = 1 and password='".$password."'");
    $c = $c->fetch_assoc();
    $dc = $con->query("select * from area_db where id>1 limit 1");
    $vb = $dc->fetch_assoc();
	$curr = $con->query("select * from setting")->fetch_assoc();
    $returnArr = array("rider"=>$c,"d_charge"=>$vb['dcharge'],"currency"=>$curr['currency'],"ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Login successfully!");
}
else
{
